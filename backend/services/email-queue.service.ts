...
export class EmailQueueService {
  ...
  private redis: Redis;
  private readonly RATE_LIMIT_HOURLY: number;
  private readonly RATE_LIMIT_DAILY: number;
  ...
  constructor() {
    ...
    this.emailQueue = new Bull<EmailJob>('email-queue', {
      redis: {
        host: redisHost,
        port: redisPort,
      },
   ...
   * Add email to queue
   */
  async queueEmail(type: 'user_confirmation' | 'admin_notification', lead: any, priority: number = 5): Promise<void> {
   ...
   */
  private getDelayUntilTomorrow(): number {
    const now = new Date();
